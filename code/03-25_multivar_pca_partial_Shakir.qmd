...
Most common: multiple Xs (predictors)  
               ...
               y ~ x1 + x2 + .... xn  
               yield ~ precip + temp + som + hybrid  [yield explained by precipitation, temperature, soil organic matter, hybrid]
...
- Based on Y (outcome) existence:
    - Supervised  [we give the y variable (aka label) i.e., we have the y variable that we are trying predict]
    - Unsupervised  [we do not have the y variable (aka label)/ no label]

- If Y exists (i.e., supervised learning), based on its type:  
    - Categorical: classification (e.g., low, high of a variable)  
    - Numerical (continuous variable): regression  
