...
```
There is no significant relationship between PC1 and "strength_gtex". [Remember: we did not give the y variable "strength_gtex" in the PCA (because it is an unsupervised method); PCA only say the weather variables.]
...
There is a significant positive relationship [slope in the summary() function was positive ] between PC2 and "strength_gtex" (although it is very noisy i.e., there is noise around the fitted curve).
...
As expected, PCs were not multicollinear (VIF=1). This is a very important feature of PCA
