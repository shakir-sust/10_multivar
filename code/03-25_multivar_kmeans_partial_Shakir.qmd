...
fviz_nbclust(weather_norm,
             ...
             k.max = 10,
             FUNcluster = kmeans #To specify what is the function that we want to use find those clusters
             )
...
mod_km4 <- kmeans(weather_norm,
                  centers = 4, #now we have 4 clusters instead of 6
                  nstart = 10
                  )
...
weather %>%
  mutate(cluster = mod_km4$cluster) %>% #to create a column with cluster id
